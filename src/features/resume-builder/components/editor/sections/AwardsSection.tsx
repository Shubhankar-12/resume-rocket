...
import type { BuilderAward, SectionEditorProps } from "../../../types";
import { TextArea, TextField } from "../fields";

export function AwardsSection({ draft, update }: SectionEditorProps) {
  const setItem = (id: string, patch: Partial<BuilderAward>) =>
    update({
      awards: draft.awards.map((x) => (x.id === id ? { ...x, ...patch } : x)),
    });

  const removeItem = (id: string) => update({ awards: draft.awards.filter((x) => x.id !== id) });
