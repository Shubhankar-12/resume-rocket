...
/**
 * FaangPath — serif, centered all-caps header, full-width ruled section headings.
 * Single-column, ATS-parseable. Pure presentational (server print + client preview).
 */
export function FaangPathTemplate({ resume }: TemplateProps) {
  const accent = accentHex(resume.accent_color);
  const { basics } = resume;
  const contact = [basics.email, basics.phone, basics.location].filter(Boolean);

  const Heading = ({ children }: { children: string }) => (
    <h2 className="mb-2 border-b border-gray-400 pb-1 text-[13px] font-bold uppercase tracking-[0.1em] text-gray-900">
      {children}
    </h2>
  );
  ...
  return (
    <div className="mx-auto max-w-[800px] bg-white px-10 py-9 font-serif text-[13px] leading-relaxed text-gray-900">
      <header className="mb-5 text-center">
        <h1 className="text-[28px] font-bold uppercase tracking-[0.08em] text-gray-900">
            ...
            resume.projects.map((x) => (
              <div key={x.id} className="mb-3">
                <p className="text-gray-800">
                  ...
                  {x.description ? " " : ""}
                  <RichText html={x.description} className="inline" />
                </p>
          ...
          {key === "certifications" &&
            resume.certifications.map((x) => (
              <div key={x.id} className="mb-1">
                ...
                {x.issuer && <div className="text-[12px] italic text-gray-700">{x.issuer}</div>}
              </div>
            ))}
          ...
          {key === "publications" &&
            resume.publications.map((x) => (
              <div key={x.id} className="mb-1.5">
                ...
                {x.publisher && (
                  <div className="text-[12px] italic text-gray-700">{x.publisher}</div>
                )}
                {x.description && <p className="mt-1 text-gray-800">{x.description}</p>}
                {x.url && (
