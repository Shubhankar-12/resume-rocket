...
export function ClassicTemplate({ resume }: TemplateProps) {
  ...
  const Heading = ({ children }: { children: string }) => (
    <h2
      className="mb-2 border-b pb-1 font-serif text-[13px] font-bold uppercase tracking-[0.12em]"
        ...
        <section key={key} className="mb-4">
          ...
          {key === "certifications" &&
            resume.certifications.map((x) => (
              <div key={x.id} className="mb-1 flex items-baseline justify-between gap-3">
                <span className="text-gray-900">
