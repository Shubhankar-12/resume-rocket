...
/**
 * Jake — the canonical clean LaTeX resume look. Serif, centered header,
 * small-caps ruled section headings (dark, not accent-colored). Single-column,
 * ATS-parseable. Pure presentational (server print page + client preview).
 */
export function JakeTemplate({ resume }: TemplateProps) {
  const accent = accentHex(resume.accent_color);
  const { basics } = resume;
  const contact = [basics.email, basics.phone, basics.location].filter(Boolean);

  const Heading = ({ children }: { children: string }) => (
    <h2 className="mb-2 border-b border-gray-400 pb-0.5 text-[13px] font-bold uppercase tracking-[0.14em] text-gray-900">
      {children}
    </h2>
  );
  ...
  return (
    <div className="mx-auto max-w-[800px] bg-white px-10 py-9 font-serif text-[13px] leading-relaxed text-gray-900">
      <header className="mb-5 text-center">
        ...
        {contact.length > 0 && (
          <p className="mt-1.5 text-[12px] text-gray-700">{contact.join("  |  ")}</p>
        )}
